...
form.addEventListener('submit', function (e) {
  ...
  try {
    ...
    SaraErrorBoundary.logError(error, '#checkoutForm submit');
...
function submitCheckoutForm() {
  ...
  resolveCartProductIds(cart)
    ...
    .then((res) => {
      ...
      const appliedPoints =
        parseInt(localStorage.getItem('sara_applied_loyalty_points'), 10) || 0;
      const currentBalance =
        parseInt(localStorage.getItem('sara_loyalty_balance'), 10) || (window.SARA_CONFIG ? window.SARA_CONFIG.LOYALTY.DEFAULT_BALANCE : 150);
      const subtotal = cart.reduce(
        (sum, item) =>
          sum + parsePriceString(item.price) * (parseInt(item.quantity, 10) || 1),
        0,
      );
      const earnedPoints = Math.floor(subtotal * (window.SARA_CONFIG ? window.SARA_CONFIG.LOYALTY.POINTS_PER_RUPEE : 10));
      const newBalance = Math.max(
        0,
        currentBalance - appliedPoints + earnedPoints,
      );
      localStorage.setItem('sara_loyalty_balance', newBalance);
      localStorage.removeItem('sara_applied_loyalty_points');
...
function renderCheckoutItems() {
  ...
  const _escStr = (s) => String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#x27;');

  container.innerHTML = cart
    .map((item) => {
      const itemPrice = parsePriceString(item.price);
      const itemQty = parseInt(item.quantity, 10) || 1;
      const sizeStr = item.size ? `Size ${_escStr(item.size)}` : 'Standard';
      const safeName = _escStr(item.name);
      const safeImg = _escStr(item.image || item.img || 'images/products/placeholder.jpg');
      return `
      <div class="order-item" style="display: flex; gap: 15px; align-items: center; border-bottom: 1px solid var(--border); padding-bottom: 12px; margin-bottom: 12px;">
        <div class="item-thumb" style="width: 50px; height: 50px; border-radius: 6px; overflow: hidden; border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; background: #fff;">
          <img src="${safeImg}" alt="${safeName}" style="max-width: 100%; max-height: 100%; object-fit: cover;" onerror="this.src='images/products/placeholder.jpg'">
        </div>
        <div class="item-info" style="flex: 1;">
          <div class="item-name" style="font-weight: 600; font-size: 14px; color: var(--color-heading);">${safeName}</div>
          <div class="item-meta" style="font-size: 12px; color: #777;">${sizeStr} · Qty ${itemQty}</div>
        </div>
        <span class="item-price-col" style="font-weight: 600; font-size: 14px; color: #C483E6;">${formatCurrency(itemPrice * itemQty * 100)}</span>
      </div>
    `;
    })
    .join('');
...
window.updateCheckoutSummary = function () {
  ...
  const urgencyDiscount = hasUrgency ? subtotal * (window.SARA_CONFIG ? window.SARA_CONFIG.URGENCY_DISCOUNT_PCT : 0.05) : 0;

  // Check gift wrap
  const hasGiftWrap = document.getElementById('gift-wrap-opt')?.checked;
  const giftCharge = hasGiftWrap ? (window.SARA_CONFIG ? window.SARA_CONFIG.GIFT_WRAP_CHARGE : 99) : 0;

  // Calculate tax
  const tax = subtotal * (window.SARA_CONFIG ? window.SARA_CONFIG.TAX_RATE : 0.18);

  // Check loyalty points discount
  const loyaltyPoints =
    parseInt(localStorage.getItem('sara_applied_loyalty_points'), 10) || 0;
  const loyaltyDiscount = loyaltyPoints / (window.SARA_CONFIG ? window.SARA_CONFIG.LOYALTY.POINTS_PER_RUPEE : 10);

  const taxCents = Math.round(subtotalCents * (window.SARA_CONFIG ? window.SARA_CONFIG.TAX_RATE : 0.18));
  ...
  if (couponCode) {
    ...
    const safeCouponCode = String(couponCode || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#x27;');
    couponRow.innerHTML = `
      <span>Discount (${safeCouponCode}) <button type="button" class="btn-remove-coupon" id="btnRemoveCoupon" aria-label="Remove coupon" style="background:none; border:none; color:#ef4444; cursor:pointer; font-weight:bold; font-size:16px; margin-left:5px; padding:0;">×</button></span>
  ...
  if (urgencyDiscountCents > 0) {
    ...
    urgencyRow.innerHTML = `<span>Urgency Promo (${window.SARA_CONFIG ? window.SARA_CONFIG.URGENCY_DISCOUNT_PCT * 100 : 5}%)</span><span id='urgency-discount-val'>-${formatCurrency(urgencyDiscountCents)}</span>`;
  } else {
  ...
  if (giftChargeCents > 0) {
    if (!giftRow) {
      ...
      giftRow.style.cssText =
        'display:flex; justify-content:space-between; margin-bottom: 8px; color: #C483E6; font-weight: 600;';
      giftRow.innerHTML =
        "<span>Gift Wrapping Service</span><span style='color: #C483E6;'>₹99.00</span>";
...
function initCheckoutPage() {
  ...
  SaraErrorBoundary.wrap('#checkoutForm', function () {
    renderCheckoutItems();
    window.updateCheckoutSummary();
  });
}
