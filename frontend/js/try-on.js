...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function injectAdjustmentSliders() {
    ...
    panel.innerHTML = `
        <h4 style="margin: 0 0 10px 0; font-size: 13px; font-weight: 700; color: #C483E6; text-transform: uppercase; letter-spacing: 0.5px;">Garment Live Fitting</h4>
        <div style="margin-bottom: 8px;">
            <label style="display:block; font-size: 11px; color:#aaa; margin-bottom:3px;">Scale Size: <span id="val-scale">1.0</span></label>
            <input type="range" id="adj-scale" min="0.8" max="1.5" step="0.05" value="1.0" style="width:100%; accent-color:#C483E6;" />
        </div>
        <div style="margin-bottom: 8px;">
            <label style="display:block; font-size: 11px; color:#aaa; margin-bottom:3px;">Vertical Offset: <span id="val-offset">0px</span></label>
            <input type="range" id="adj-offset" min="-50" max="50" step="2" value="0" style="width:100%; accent-color:#C483E6;" />
        </div>
        <div style="margin-bottom: 8px;">
            <label style="display:block; font-size: 11px; color:#aaa; margin-bottom:3px;">Fabric Opacity: <span id="val-opacity">90%</span></label>
            <input type="range" id="adj-opacity" min="0.5" max="1.0" step="0.05" value="0.90" style="width:100%; accent-color:#C483E6;" />
        </div>
    `;
  ...
  function downloadResult() {
    ...
    link.download = 'sara-virtual-tryon.png';
  ...
  function shareResult() {
    if (navigator.share) {
      resultCanvas.toBlob((blob) => {
        const file = new File([blob], 'sara-tryon.png', { type: 'image/png' });
        navigator
          .share({
