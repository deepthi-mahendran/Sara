...
function renderTimeline() {
    ...
    let html = `
        <div style="display:flex; justify-content:space-between; margin: 30px 0; font-family:sans-serif; position:relative;">
            ...
            <div id="timeline-bar" style="position:absolute; top:12px; left:0; width:${percent}%; height:4px; background:#C483E6; z-index:2; transition:width 0.5s;"></div>
    `;

    stages.forEach((stage, idx) => {
        const isActive = idx <= safeIndex;
        const bg = isActive ? '#C483E6' : '#ccc';
        const color = 'white';
