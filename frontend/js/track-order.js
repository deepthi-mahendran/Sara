/**
 * track-order.js
 * Handles the Track My Order page functionality for the Sara e-commerce project.
...
if (form) {
  form.addEventListener('submit', function (e) {
    ...
    if (!orderIdRaw) {
      ...
      isTrackValid = false;
    } else if (!/^SARA-\d+$/.test(orderIdRaw)) {
      showTrackError(
        orderIdInput,
        'Invalid Format. Pattern should be SARA-XXXXXXXX (e.g. SARA-20261234)',
      );
      isTrackValid = false;
    }
...
function renderResult(order) {
  ...
  if (!liveContainer) {
    ...
    liveContainer.innerHTML = `
      <div style="display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 6px; font-weight: 600; color: #C483E6;">
      ...
      <div style="background: rgba(0,0,0,0.1); height: 8px; border-radius: 4px; overflow: hidden; position: relative;">
        <div id="liveProgressBar" style="background: #C483E6; height: 100%; width: 62%; transition: width 1s linear;"></div>
      </div>
