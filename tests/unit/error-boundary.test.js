...
import '../../frontend/js/error-boundary.js';
...
describe('error-boundary.js unit tests', () => {
  ...
  it('wrap calls renderFn successfully and does not log an error', () => {
    ...
    SaraErrorBoundary.wrap('#test-target', renderFn);
  ...
  it('wrap renders a fallback div when renderFn throws', () => {
    ...
    SaraErrorBoundary.wrap('#test-target', renderFn);
    const fallback = container.querySelector('.sara-error-fallback');
  ...
  it('wrap adds a retry button that re-executes the renderFn', () => {
    ...
    const retryBtn = container.querySelector('.sara-error-retry');
  ...
  it('wrap returns early when container is not found', () => {
    const renderFn = vi.fn();
    SaraErrorBoundary.wrap('#nonexistent-target', renderFn);
    expect(renderFn).not.toHaveBeenCalled();
  });

  it('logError does not emit to console.error (silent hook)', () => {
    const err = new Error('test error');
    SaraErrorBoundary.logError(err, 'my-context');
  ...
  it('wrap renders the fallback message when renderFn throws', () => {
    ...
    expect(fallback.querySelector('.sara-error-retry')).not.toBeNull();
  });
  ...
  it('wrap leaves the container untouched when renderFn succeeds', () => {
    ...
    expect(container.querySelector('.sara-error-fallback')).toBeNull();
  });
  ...
  it('retry replaces the fallback with the rendered content on success', () => {
    ...
    SaraErrorBoundary.wrap('#test-target', renderFn);
    expect(container.querySelector('.sara-error-fallback')).not.toBeNull();

    container.querySelector('.sara-error-retry').click();
    expect(callCount).toBe(2);
